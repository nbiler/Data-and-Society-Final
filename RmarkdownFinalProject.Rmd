...
drug_df %>% 
  ...
  filter(complete.cases(.)) %>% 
   mutate(ILLMON_LAB = factor(case_when(
    ILLMON == 0 ~ "Has not used in last month",
    ILLMON == 1 ~ "Used in last month"
  ), levels = c("Has not used in last mont", "Used in last month")),
  POVERTY3_LAB = factor(case_when(
    POVERTY3 == 1 ~ "Living in poverty",
    POVERTY3 == 2 ~ "Income above Federal Poverty Threshold",
    POVERTY3 == 3 ~ "Income above Federal Poverty Threshold"),
    levels = c("Living in poverty", "Income above Federal Poverty Threshold"))) %>% 
  group_by(POVERTY3_LAB, ILLMON_LAB) %>% 
  tally() %>% 
  mutate(prop_pov = prop.table(n)) %>% 
  filter(ILLMON_LAB == "Used in last month") %>% 
